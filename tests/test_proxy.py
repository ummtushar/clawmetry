...
def proxy_config():
    ...
    config = ProxyConfig(
        ...
        loop_detection=LoopDetectionConfig(enabled=True, window_seconds=300, max_similar=3),
    )
    config.providers = {
        "anthropic": MagicMock(api_key_env="ANTHROPIC_API_KEY", base_url="https://api.anthropic.com"),
        "openai": MagicMock(api_key_env="OPENAI_API_KEY", base_url="https://api.openai.com"),
    }
    return config
...
class TestRequestHashing:
    ...
    def test_different_content_different_hash(self):
        from clawmetry.proxy import compute_request_hash
        body1 = {"model": "claude-opus-4", "messages": [{"role": "user", "content": "Hello"}]}
        body2 = {"model": "claude-opus-4", "messages": [{"role": "user", "content": "World"}]}
    ...
    def test_system_prompt_affects_hash(self):
        from clawmetry.proxy import compute_request_hash
        body1 = {"model": "x", "system": "Be helpful", "messages": [{"role": "user", "content": "Hi"}]}
        body2 = {"model": "x", "system": "Be mean", "messages": [{"role": "user", "content": "Hi"}]}
        assert compute_request_hash(body1) != compute_request_hash(body2)
...
class TestProviderDetection:
    ...
    def test_anthropic_by_header(self):
        from clawmetry.proxy import detect_provider
        assert detect_provider("/v1/unknown", {"x-api-key": "sk-ant-..."}, {}) == "anthropic"

    def test_openai_by_auth_header(self):
        from clawmetry.proxy import detect_provider
        assert detect_provider("/v1/unknown", {"authorization": "Bearer sk-abc123"}, {}) == "openai"

    def test_anthropic_by_model(self):
        from clawmetry.proxy import detect_provider
        assert detect_provider("/v1/unknown", {}, {"model": "claude-opus-4"}) == "anthropic"
...
class TestProxyDB:
    def test_record_and_query_usage(self, proxy_db):
        proxy_db.record_usage(
            ...
            session_id="test-session", request_hash="abc123",
        )
    ...
    def test_record_and_query_events(self, proxy_db):
        proxy_db.record_event("budget_blocked", "Over budget", severity="warning",
    ...
    def test_request_hashes(self, proxy_db):
        for i in range(5):
            proxy_db.record_usage(
                ...
                session_id="session-1", request_hash=f"hash_{i % 2}",
            )
...
class TestBudgetEnforcer:
    ...
    def test_blocks_over_daily_budget(self, budget_enforcer, proxy_db):
        proxy_db.record_usage(
            provider="anthropic", model="test",
            input_tokens=100, output_tokens=50, cost_usd=11.0,
        )
        allowed, reason = budget_enforcer.check()
    ...
    def test_blocks_over_monthly_budget(self, proxy_db):
        ...
        proxy_db.record_usage(
            provider="anthropic", model="test",
            input_tokens=100, output_tokens=50, cost_usd=101.0,
        )
    ...
    def test_allows_when_no_limits(self, proxy_db):
        ...
        proxy_db.record_usage(
            provider="anthropic", model="test",
            input_tokens=100, output_tokens=50, cost_usd=999.0,
        )
...
class TestLoopDetector:
    def test_no_loop_with_few_requests(self, loop_detector, proxy_db):
        proxy_db.record_usage(
            provider="anthropic", model="test",
            input_tokens=100, output_tokens=50, cost_usd=0.01,
            session_id="s1", request_hash="abc123",
        )
    ...
    def test_detects_loop(self, loop_detector, proxy_db):
        for _ in range(4):
            proxy_db.record_usage(
                provider="anthropic", model="test",
                input_tokens=100, output_tokens=50, cost_usd=0.01,
                session_id="s1", request_hash="same_hash",
            )
    ...
    def test_different_hashes_no_loop(self, loop_detector, proxy_db):
        for i in range(10):
            proxy_db.record_usage(
                provider="anthropic", model="test",
                input_tokens=100, output_tokens=50, cost_usd=0.01,
                session_id="s1", request_hash=f"unique_{i}",
            )
    ...
    def test_disabled_loop_detection(self, proxy_db):
        ...
        detector = LoopDetector(
            LoopDetectionConfig(enabled=False), proxy_db
        )
        for _ in range(10):
            proxy_db.record_usage(
                provider="anthropic", model="test",
                input_tokens=100, output_tokens=50, cost_usd=0.01,
                session_id="s1", request_hash="same",
            )
...
class TestModelRouter:
    ...
    def test_session_match_routing(self):
        from clawmetry.proxy import ModelRouter, RoutingRule
        router = ModelRouter([
            RoutingRule(match_session="subagent", target_model="claude-3-haiku-20240307"),
        ])
    ...
    def test_combined_match(self):
        ...
        router = ModelRouter([
            RoutingRule(
                match_model="opus", match_session="subagent",
...
class TestProxyConfig:
    ...
    def test_save_and_load(self, tmp_path):
        ...
        with patch("clawmetry.proxy.PROXY_CONFIG_FILE", config_file):
            ...
            config_file.write_text(json.dumps({
                "port": 5000,
                "budget": {"daily_usd": 25.0, "monthly_usd": 200.0, "action": "warn"},
            }))
    ...
    def test_env_override(self, tmp_path):
        ...
        config_file = tmp_path / "proxy.json"
        with patch("clawmetry.proxy.PROXY_CONFIG_FILE", config_file), \
             patch.dict(os.environ, {"CLAWMETRY_PROXY_PORT": "9999", "CLAWMETRY_PROXY_DAILY_USD": "50.0"}):
...
class TestProxyApp:
    ...
    def client(self, tmp_path):
        from clawmetry.proxy import create_proxy_app, ProxyConfig, BudgetConfig, LoopDetectionConfig
        config = ProxyConfig(
            port=14100,
            budget=BudgetConfig(daily_usd=10.0, monthly_usd=100.0, action="block"),
            loop_detection=LoopDetectionConfig(enabled=True, max_similar=3, window_seconds=300),
        )
